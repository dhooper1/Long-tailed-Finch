...
#Usage - Generates a BED file for each molecule from a BX-sorted BAM file, defined as reads that share the same BX tag with a maximum internal gap of 50kb
...
chomp($ARGV[0]);
my $outfile = $ARGV[0];
$outfile=~s/\.bam/.linked_reads.full.bed/;
open(OUT, ">$outfile");
	...
	if (($bx ne $last_bx) || ($temp[2] ne $last_chr)) {
		...
		if (@cache) {
			...
			MOL: foreach my $m (0..$#mols) {
				...
				print OUT "$mchr_out\t$mstart\t$mend\t$mname\t$mqual\t$mori\t$mtstart\t$mtend\t$mcol\t".(@starts)."\t$msizes_str\t$mstarts_str\n";
			}
		}
		undef @cache;
	};
