...
if len(sys.argv) != 5:
    print("Usage: python imputation_accuracy_write2out.py truth_genotypes.txt imputed_genotypes.txt <INFO_SCORE_threshold> output_results.tsv")
    sys.exit(1)
...
for i in range(len(bins) - 1):
    ...
    if num_sites_in_bin > 0:
        if bin_df['GT_truth'].nunique() > 1 and bin_df['GT_imputed'].nunique() > 1:
            # Calculate r^2 for the bin
            r, _ = pearsonr(bin_df['GT_truth'], bin_df['GT_imputed'])
...
if merged_df['GT_truth'].nunique() > 1 and merged_df['GT_imputed'].nunique() > 1:
    r_all, _ = pearsonr(merged_df['GT_truth'], merged_df['GT_imputed'])
